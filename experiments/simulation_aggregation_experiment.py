...
from voting import build_decision_pools, generate_pool_outcomes
from aggregation_rules import agent_majority_decision, pool_majority_decision, accuracy_weighted_pool_decision


def one_aggregation_simulation(
    ...
    # Alternative aggregation rules use separate random streams for tie-breaking.
    pool_rng = np.random.default_rng(None if seed is None else 1000000 + seed)
    weighted_rng = np.random.default_rng(None if seed is None else 2000000 + seed)
    ...
    task_state_history = []
    agent_majority_decision_history = []
    pool_majority_decision_history = []
    accuracy_weighted_decision_history = []

    agent_majority_accuracy_history = []
    pool_majority_accuracy_history = []
    accuracy_weighted_accuracy_history = []
    ...
    for t in range(T):
        ...
        # Generate one common set of pools, recommendations and submitted votes.
        ...
        # Compare the three aggregation rules using the same submitted votes.
        agent_decision = agent_majority_decision(votes=votes, rng=rng)
        pool_decision = pool_majority_decision(pools=pools, votes=votes, rng=pool_rng)
        weighted_decision = accuracy_weighted_pool_decision(
            pools=pools,
            votes=votes,
            gamma=gamma,
            alpha=alpha,
        ...
        agent_correct = int(agent_decision == theta)
        pool_correct = int(pool_decision == theta)
        weighted_correct = int(weighted_decision == theta)

        submitted_vote_accuracy = float(np.mean(vote_correct))

        # Belief updating remains unchanged.
        ...
        agent_majority_decision_history.append(agent_decision)
        pool_majority_decision_history.append(pool_decision)
        accuracy_weighted_decision_history.append(weighted_decision)

        agent_majority_accuracy_history.append(agent_correct)
        pool_majority_accuracy_history.append(pool_correct)
        accuracy_weighted_accuracy_history.append(weighted_correct)
    ...
    results = {
        ...
        "agent_majority_decision": np.asarray(agent_majority_decision_history),
        "pool_majority_decision": np.asarray(pool_majority_decision_history),
        "accuracy_weighted_decision": np.asarray(accuracy_weighted_decision_history),

        "agent_majority_accuracy": np.asarray(agent_majority_accuracy_history),
        "pool_majority_accuracy": np.asarray(pool_majority_accuracy_history),
        "accuracy_weighted_accuracy": np.asarray(accuracy_weighted_accuracy_history),
        ...
        "mean_agent_majority_accuracy": float(np.mean(agent_majority_accuracy_history)),
        "mean_pool_majority_accuracy": float(np.mean(pool_majority_accuracy_history)),
        "mean_accuracy_weighted_accuracy": float(np.mean(accuracy_weighted_accuracy_history)),
